function mergeDataStreams(stream1, stream2) {
  ...
  {
    myArray[stream1[i].id] = stream1[i];
  }

  for(var key in myArray)
  {
    var foundData = stream2.find(function(item) {return item.id === key;});
    for (var _key in foundData)
    {
      results[key][_key] = foundData[_key];
    }
  }
  return Object.keys(myArray).map(function(item) {return myArray[item];});
  // your code here
}
